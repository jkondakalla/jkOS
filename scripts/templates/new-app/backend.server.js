'use strict';
// __NAME__ backend — scaffolded by `pnpm new-app`.
//
...
const DB_PATH    = process.env.DB_PATH    || path.join(__dirname, '__DB__');
...
/* Cross-origin allowlist — SHELL_URL plus any ALLOWED_ORIGINS (comma-separated), so a
   second suite app can call __NAME__ cross-origin. (Under the same-origin edge, peer
...
/* Write authorization — the shared weave gate (guest read-only → service NO_USER_CONTEXT
   → __ID__:write scope; a delegated service token writes per-user). Reads need no extra
   gate; every row is scoped to req.user.sub by the collection. */
app.use(weaveWriteGate({ scope: '__ID__:write' }));

/* ── Health ────────────────────────────────────────────────────────────── */
app.get('/health', healthHandler('__ID__'));

/* ── Weave discovery declarations ───────────────────────────────────────────
   What can be DONE to (CAPABILITIES) and READ from (DATASETS) __NAME__, derived from
...
function boot() {
  runMigrations();
  app.listen(PORT, () => console.log(`__NAME__ running on :${PORT}`));
}

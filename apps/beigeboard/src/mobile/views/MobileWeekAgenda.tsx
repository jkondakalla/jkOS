/**
 * MobileWeekAgenda — BeigeBoard's phone week: a vertical 7-day list.
 *
 * This body used to live inside @jkos/cards' WeekView, behind a
 * `useBreakpoint() === 'mobile'` branch. It is on v0 styles (hardcoded rgba
 * fills, a colour bar down each row's left edge) that the suite has moved off,
 * and while it sat inside a kit component it rendered any time the WINDOW was
 * narrow — including inside ORDECK's bb-week widget and every design-system
 * preview of <WeekView>. The kit is the design spec, so the un-migrated body
 * moved here, to the only app that asks for it. @jkos/cards' WeekView is now
 * the grid at every width.
 *
...
export function MobileWeekAgenda({ items, today, resolvers, onSelect, onToggle }: WeekViewProps) {

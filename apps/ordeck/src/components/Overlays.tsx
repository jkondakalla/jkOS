...
/* Suite-wide CRT overlay effects for ORDECK.
   Defaults: grain + halation only. Scanlines and artifacts are user opt-in. */

interface FilmGrainProps {
  strength?: number;
}

export function FilmGrain({ strength = 0.07 }: FilmGrainProps) {
  return (
    <svg
      style={{
    ...
    >
      ...
      <rect width="100%" height="100%" filter="url(#hub-fg)" />
    </svg>
  );
}

/* Halation — reuses the existing #halation filter defined in global.css/ORDECK's canvas.
   This version is a standalone SVG definition that can be used separately. */
export function Halation() {
  return (
    <svg style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }} aria-hidden="true">
      <defs>
        <filter id="hub-halation" x="-8%" y="-8%" width="116%" height="116%" colorInterpolationFilters="sRGB">
...
export function ScanLines({ strength = 1 }: ScanLinesProps) {
  const lineColor = `rgba(255,255,255,${0.014 * strength})`;
  return (
    <div
      aria-hidden="true"
      style={{
...
function makeArtifact() {
  ...
  if (isScratch) {
    ...
    return { id, type: 'scratch', x: left ? 1 + Math.random() * 11 : 88 + Math.random() * 10, y: 3 + Math.random() * 40, height: 10 + Math.random() * 32, tilt: (Math.random() - 0.5) * 3.5, peakOp: 0.28 + Math.random() * 0.42, dur: 90 + Math.floor(Math.random() * 240), bright };
  }
  const lx = Math.random() < 0.5, ty = Math.random() < 0.5;
  return { id, type: 'blip', x: lx ? 1 + Math.random() * 12 : 87 + Math.random() * 12, y: ty ? 1 + Math.random() * 12 : 87 + Math.random() * 12, w: 2 + Math.floor(Math.random() * 5), h: 2 + Math.floor(Math.random() * 5), peakOp: 0.45 + Math.random() * 0.45, dur: 55 + Math.floor(Math.random() * 160), bright };
}
...
export function Artifacts() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 9997 }}>
      {items.map(a => {
        const color = a.bright ? 'var(--hub-cream-bright)' : 'var(--hub-bg-1)';

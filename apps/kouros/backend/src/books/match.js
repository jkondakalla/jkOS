...
const { extractYear, cleanGenres } = require('./probe');   // same 4-digit-year convention scan.js's mapTagsToColumns uses
...
async function applyCandidate(db, dataDir, doFetch, book, candidate) {
  ...
  if (isFetchableCoverUrl(candidate.cover)) {
    try {
      ...
      console.warn(`[kouros books] applyCandidate artwork download failed for book ${book.id}: ${err.message}`);
    }
  }
 ...
 *  it automatically after every scan (KOUROS_BOOKS_AUTO_ENRICH=1 — Jag 2026-07-10: "all
...
async function runEnrichmentSweep({ db, dataDir, doFetch, cap = 50, delayMs = 250 }) {
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    try {
      ...
      console.warn(`[kouros books] enrichment search failed for book ${book.id}: ${err.message}`);
    ...
    if (pick) {
      try {
        ...
        console.error(`[kouros books] enrichment apply failed for book ${book.id}: ${err.message}`);
...
function createMatchRouter({ db, dataDir, fetch: injectedFetch }) {
  ...
  router.post('/api/match', async (req, res) => {
    ...
    try {
      ...
      console.error(`[kouros books] matchBook metadata write failed for book ${id}: ${err.message}`);
  ...
  // The capability doc (discovery.js) still declares scopes:['kouros:admin'] — role
  ...
  router.post('/api/match/all', async (req, res) => {
    ...
    // the identical policy automatically after every scan (KOUROS_BOOKS_AUTO_ENRICH).

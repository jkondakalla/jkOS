...
//     pure and unit-tested (books.probe.smoke.mjs).
//
// ⚠️ `dataDir` here is the BOOKS data dir (<DATA_DIR>/books), not DATA_DIR itself. The
// brick writes covers to `<dataDir>/covers/<id>.jpg`, and tracks already own
// `<DATA_DIR>/covers/<id>.jpg` — book 12 and track 12 would overwrite each other's art.
// Everything that resolves a book's `cover_path` (books/media.js, books/match.js) is
// handed the same books data dir, so the relative `covers/12.jpg` it stores resolves.
 ...
 */
function createBookScanner({ db, audiobooksDir, dataDir, concurrency = 4, onScanComplete } = {}) {
  if (!db) throw new Error('createBookScanner: db is required');
  if (!audiobooksDir) throw new Error('createBookScanner: audiobooksDir is required');
  if (!dataDir) throw new Error('createBookScanner: dataDir is required');

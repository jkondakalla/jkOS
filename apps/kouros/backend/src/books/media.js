...
// process those collide — book 12 and track 12 would answer on the same URL, and
// whichever router mounted first would win silently. `spec.routes` moves the book
// half to `/api/books/stream|cover|download`; the detail route below stays at
// `/api/book/:bookId`, its declared path. Everything generic — range-aware
...
// book-SPECIFIC config: how a bookId maps to files on disk (WITH containment), the
...
//   cover_path   = RELATIVE to the BOOKS data dir (e.g. "covers/12.jpg" under
//                  <DATA_DIR>/books — see books/scan.js for why it is not DATA_DIR).
...
const { cleanGenres } = require('./probe');                // serve-time noise-genre filter
...
/* ── The Firefox-m4b compat ladder — app-supplied config, not brick literals ──────
   ...
   <booksDataDir>/compat/<bookId>-<fileIndex>.c<level>.m4a — the brick's default variantName +
...
/** Where the book half of the brick answers — see the file header for the collision
 *  this avoids. The frontend's book source (src/player/sources.ts) builds its URLs from
 *  the same three strings, so the two halves are named once each. */
const BOOK_ROUTES = Object.freeze({
  stream: '/api/books/stream',
  cover: '/api/books/cover',
  download: '/api/books/download',
});
...
/* A module-level reference to the brick instance so prepareAllCompat (the post-scan
   ...
   two ffmpegs for the same variant. createBookMediaRouter is called exactly once at boot. */
let sharedMedia = null;

/**
 * @param {{ db: import('better-sqlite3').Database, audiobooksDir: string, dataDir: string }} deps
 */
function createBookMediaRouter({ db, audiobooksDir, dataDir }) {
  if (!db) throw new Error('createBookMediaRouter: db is required');
  if (!audiobooksDir) throw new Error('createBookMediaRouter: audiobooksDir is required');
  if (!dataDir) throw new Error('createBookMediaRouter: dataDir is required');
  ...
  const media = defineMediaRoutes({
    ...
    archiver,   // a multi-file book downloads zipped; the brick never hard-depends on it
    onError: (ctx, err) => console.error(`[kouros books] ${ctx}: ${err && err.message ? err.message : err}`),
  });
  sharedMedia = media;
 ...
 *  Fired after each book scan by server.js (KOUROS_BOOKS_AUTO_COMPAT=1 — compose-only,
 *  never in tests: books.playback.smoke asserts 404-before-prepare, which a background
 *  sweep would race).
...
module.exports = { createBookMediaRouter, prepareAllCompat, BOOK_ROUTES };

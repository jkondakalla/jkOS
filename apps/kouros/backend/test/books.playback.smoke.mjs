...
//
//   node apps/kouros/backend/test/books.playback.smoke.mjs
...
// Claimed in the suite-manifest port registry ('kouros:books.playback.smoke') — the
// `port-registry` probe holds this literal to that claim. It used to be 3991,
// shared with BeigeBoard's routines.smoke — that overlap is exactly OPS-1.
const PORT = 3997;
...
const { tmp, ok, boot, crashed, done } = smoke('books.playback.smoke');
...
const A = mkToken({ sub: 301, role: 'admin', scope: ['kouros:write'] });
const B = mkToken({ sub: 302, role: 'admin', scope: ['kouros:write'] });
...
async function waitForCompatReady(token, bookId, fileIndex, level, ms = 20000) {
  ...
  let last = null;
  while (Date.now() < deadline) {
    last = await req('POST', `/api/books/stream/${bookId}/${fileIndex}/prepare`, { level }, token);
...
try {
  ...
  const anonStream = await fetch(BASE + '/api/books/stream/1/0');
  ok(anonStream.status === 401, `auth: unauthenticated GET /api/stream → 401 (got ${anonStream.status})`);
  const anonCover = await fetch(BASE + '/api/books/cover/1');
  ...
  if (bookA) {
    ...
    const rangeRes = await fetch(BASE + `/api/books/stream/${bookA.id}/0`, {
    ...
    const wholeRes = await fetch(BASE + `/api/books/stream/${bookA.id}/0`, { headers: { Authorization: `Bearer ${A}` } });
    ...
    const missing = await fetch(BASE + '/api/books/stream/999999/0', { headers: { Authorization: `Bearer ${A}` } });
  ...
  if (bookB) {
    ...
    const coverRes = await fetch(BASE + `/api/books/cover/${bookB.id}`, { headers: { Authorization: `Bearer ${A}` } });
    ...
    if (bookA) {
      const noCoverRes = await fetch(BASE + `/api/books/cover/${bookA.id}`, { headers: { Authorization: `Bearer ${A}` } });
  ...
  //    variant. BOOKS_DATA_DIR mirrors server.js's own `<dirname(DB_PATH)>/books` —
  //    DB_PATH is `<tmp>/test.db`, so compat variants land under `<tmp>/books/compat/`
  //    (the books half keeps its covers and variants apart from the tracks').
  // =====================================================================================
  if (bookA) {
    const fixtureFilePath = join(FIXTURES_DIR, 'Fixture Book A', 'book.m4b');
    const DATA_DIR = join(tmp, 'books');
    ...
    const badPrepare = await req('POST', `/api/books/stream/${bookA.id}/0/prepare`, { level: 3 }, A);
    ok(badPrepare.status >= 400 && badPrepare.status < 500,
      `compat: prepare with an out-of-whitelist level → 4xx (got ${badPrepare.status})`);
    const badCompatGet = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=3`, { headers: { Authorization: `Bearer ${A}` } });
    ...
    const notYet = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=1`, { headers: { Authorization: `Bearer ${A}` } });
    ok(notYet.status === 404, `compat: ?compat=1 before any prepare call → 404 (got ${notYet.status})`);

    // prepare → poll → ready.
    const firstPrepare = await req('POST', `/api/books/stream/${bookA.id}/0/prepare`, { level: 1 }, A);
    ...
    const rePrepare = await req('POST', `/api/books/stream/${bookA.id}/0/prepare`, { level: 1 }, A);
    ...
    const compatWhole = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=1`, { headers: { Authorization: `Bearer ${A}` } });
    ...
    const compatRange = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=1`, {
    ...
    const staleGet = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=1`, { headers: { Authorization: `Bearer ${A}` } });
    ...
    const regenGet = await fetch(BASE + `/api/books/stream/${bookA.id}/0?compat=1`, { headers: { Authorization: `Bearer ${A}` } });

...
//
//   node apps/kouros/backend/test/books.meta.smoke.mjs
...
const PRELOAD_PATH = join(__dirname, 'fixtures', 'books', 'meta', 'fetch-mock-preload.cjs');
const { ITUNES_ITEM, FAKE_JPEG_MARKER } = require('./fixtures/books/meta/fetch-mock-data.cjs');

// Claimed in the suite-manifest port registry ('kouros:books.meta.smoke') — the
...
const { tmp, ok, boot, crashed, done, log } = smoke('books.meta.smoke');
...
const ADMIN = mkToken({ sub: 401, role: 'admin', scope: ['kouros:admin', 'kouros:write'] });
const USER = mkToken({ sub: 402, role: 'user', scope: ['kouros:write'] });
...
try {
  ...
  ok(log().includes('[fetch-mock-preload] globalThis.fetch replaced'),
  ...
  if (bookA && candidate) {
    ...
    // discovery.js comment: scopes:['kouros:write'], not ['kouros:admin']).
    ...
    if (after.json?.cover_path) {
      const coverAbsPath = join(tmp, 'books', after.json.cover_path);   // the BOOKS data dir: <dirname(DB_PATH)>/books
      let coverBytes = null;

...
//      string, ms_played, completed).
//   3. A SECOND create for the same (user, item_ref) APPENDS a second row — no
//      upsert/collapse (the opposite contract from `ratings`' UNIQUE + upsert trigger).
//   4. PATCH /api/history/:id and DELETE /api/history/:id → 404 — the routes are NOT
//      MOUNTED at all (defineCollection's `only` option omits them entirely).
...
// Claimed in the suite-manifest port registry ('kouros:history.smoke') — the
...
try {
  ...
  // ── 3. a SECOND create for the same track APPENDS — no upsert/collapse ────────

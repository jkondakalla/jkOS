...
| `pnpm check:tokens` \| `check:nginx` \| `check:responsive` \| `check:drag` \| `check:cards` \| `check:routine` \| `check:hud` \| `check:docker` \| `check:async-view` \| `check:overlay` \| `check:design` \| `check:fields` \| `check:scroll` \| `check:text` \| `check:today` \| `check:refs` \| `check:binding` \| `check:columns` \| `check:rulings` \| `check:audit` \| `check:secrets` \| `check:policy` \| `check:auth` \| `check:docs` | **24 individual conformance gates** (§2.2) — each is also inside `test:contracts`. ⚠️ This line said *fifteen* for eight gates longer than it was true; `check:docs` now holds the number to the `check:*` scripts that actually exist. |

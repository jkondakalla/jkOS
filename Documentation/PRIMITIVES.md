...
| `pnpm check:tokens` \| `check:token-identity` \| `check:nginx` \| `check:responsive` \| `check:drag` \| `check:cards` \| `check:routine` \| `check:hud` \| `check:docker` \| `check:async-view` \| `check:overlay` \| `check:design` \| `check:fields` \| `check:scroll` \| `check:text` \| `check:today` \| `check:refs` \| `check:binding` \| `check:pulsarmap` \| `check:vibespace` \| `check:session` \| `check:scene` \| `check:runes` \| `check:columns` \| `check:rulings` \| `check:scopes` \| `check:audit` \| `check:secrets` \| `check:policy` \| `check:auth` \| `check:docs` \| `check:build` | **32 individual conformance gates** (§2.2) — each is also inside `test:contracts`. ⚠️ This line said *fifteen* for eight gates longer than it was true; `check:docs` now holds the number to the `check:*` scripts that actually exist. |
...
| `pnpm check:*` × 32 | see §2.2 | static conformance + `check:build` (a real `vite build` ×3) |
...
| `pnpm check:session` | `test/session-client.mjs` — the pure logic under KourOS's listening-session client (`apps/kouros/src/session/`): the SSE framing (⚠️ a CRLF split MID-event must not read as a blank line that cuts one event in two), the reconnect backoff, the server clock and the position a REMOTE device extrapolates (never backwards, never past the end), the sleep countdown, **the one "what is this tab?" decision** (`modeOf` — this device the output but another of its tabs plays it is a REMOTE, never a second copy), and the default device name. Every one of these fails silently, on another device. |

...
| `items.smoke.mjs` | 71 | Direct CRUD hardening (plus **§O**, the activity contract, and **§P/§Q**, the dependency graph and the planner's missing facts — D6/D12): user scoping (A can't touch B), parent-cycle rejection, cascade delete, the reserved-source guard (`source:'google'` → 400), cap/date validation parity, service identities don't trigger the demo seed, OAuth-callback public-path behaviour, AI output sanitisation (mock LazurOS booted in-harness). |
...
| `pnpm check:build` (`vite build` ×4) | **Every SPA actually builds.** ORDECK, BeigeBoard, PapyrOS, KourOS — `tsc` + `vite build` each, ~5 s total. ⚠️ **Added 2026-09-10 because the gate had no idea whether the suite could be shipped.** BeigeBoard's production build was dead — `@jkos/routine-spec` missing from `commonjsOptions.include`, so rollup could not synthesize the CJS default export — and `pnpm test:contracts` stayed **green through all of it**, because it ran every test, every static check and the prober and never once ran `build`. A gate that cannot tell you the app is unbuildable is green about the wrong thing. Verified to go red when the include is removed. SylibOS is deliberately excluded (separate track). |

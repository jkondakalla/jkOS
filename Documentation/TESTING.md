...
| `lego.mjs` | 83 | The Layer-D bricks: `defineCollection` (ddl/docs/mount coherence), `defineConnector`, trigger engine + typed-stud validation, delegation plumbing. |
...
| `playback.smoke.mjs` | 41 | The playback backend (task 3.5): boots the real server with a REAL RS256 keypair (forged per-user tokens, not the single-identity dev stub) against the fixture library. Owner-scoped `progress` round-trip as two mock users (A/B never see each other's rows; cross-user PATCH/DELETE 404; a real position-bump PATCH; the `finished` boolean filter); range-aware `GET /api/stream/:bookId/:fileIndex` (`Range: bytes=0-1023` → 206 with the true `Content-Range`/`Content-Length`/body-length trio, computed off the actual file size, never hardcoded); `GET /api/cover/:bookId` → 200 against a real folder-level `cover.jpg` added to Fixture Book B (`gen-fixtures.sh`); an unauthenticated media request → 401 (pins 3.4's mount-ordering fix). **Two BUGs found and pinned, not fixed** (root cause is in `packages/weave/src/server/collection.js`/`filters.js`, out of scope for this task): (a) a `type: 'ref'` field (e.g. `book_ref`) is stored/returned as `"<id>.0"` instead of `"<id>"` — a numeric ref gets TEXT-column affinity but is never stringified before binding, so better-sqlite3's REAL bind + SQLite's REAL→TEXT conversion mangles it; (b) `?finished=true|false` (the wire contract `discovery.js`'s own comment documents) always matches zero rows — the `'eq'` filter binds the raw query string with no boolean coercion against an INTEGER-affinity column, so `TEXT 'true'` never equals `INTEGER 1`; `?finished=1`/`?finished=0` work correctly. Same `ffprobe` PATH requirement/skip behavior as `library.smoke.mjs`. |

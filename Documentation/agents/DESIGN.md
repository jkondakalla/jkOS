...
| **KourOS** (music + audiobooks) | React 18, plain CSS | Fraunces (suite default) | `accent: { #4b3f8f, #dba13c }`, `radius: { base 5, xs 3, sm 4, lg 8, soft 6, button 5 }` | The `@jkos/player` consumer for both kinds — PapyrOS (the first consumer, its own terracotta theme) folded into it 2026-09-23, bringing the offline cache + SW media; loads Fraunces since Full Press (the primitives are printed) |

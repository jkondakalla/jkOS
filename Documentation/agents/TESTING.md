...
| `lifecycle.mjs` | 26 | The session lifecycle through the REAL verify→write-gate chain: every token's lifetime read from config, never re-typed (the access token lives exactly `ACCESS_TTL_MS`; the client-credentials `expires_in` equals `exp − iat`); silent refresh + rotation reuse detection, expiry, guest `READ_ONLY`, service `NO_USER_CONTEXT`, on-behalf-of delegation lands as the acting user, python-jose cross-verify of real tokens. |
...
| `pnpm check:docker` (`test/dockerfile-inject.mjs`) | Every app Dockerfile that builds a frontend after `COPY . .` re-runs `pnpm install` first — so an injected `packages/*` workspace dep (e.g. `@jkos/weave`) doesn't build against a stale, manifest-only install. Root-caused a real wave-6 deploy break (2026-07-09, TS2307) before this gate existed. Also: every `packages/*` copied before a `pnpm deploy` brings its workspace deps, and every compose build whose Dockerfile copies its whole context is read an ignore file with `**/.env` (the file Docker reads, derived from the compose `context:`; a `.dockerignore` nothing reads fails). |
...
| `pnpm check:auth` (`test/auth-single-source.mjs`) | One session state machine for the suite: `@jkos/auth-client`'s `useAuthProvider` owns it, the bootstrap order (`getMe` → `refreshToken` → retry → logged-out) survives, and no file under `apps/*/src` re-declares it (defines `useAuth`/`useAuthProvider`, makes an auth context, or calls `getMe`/`refreshToken`) — it once read two named re-export files, so a fork anywhere else passed. And **logout is one function that always leaves**: `auth-client`'s `logout()` redirects even when its POST fails (it once awaited the POST bare, so a network error left "Log out" doing nothing), and no frontend hand-rolls `POST /auth/logout` — the `@jkos/ui` drawer is the one named exemption, since it cannot import `auth-client`. |
...
| `pnpm check:fields` | `test/fields.mjs` — every input routes through the one `.jk-field` primitive, not an app-local input dialect; `appearance` is reset everywhere a control is drawn by hand. **KourOS is its one named exemption** (glass-drawn inputs; converging them is Stage F visual work — TODO §6). And **no inline `outline: 'none'`** in any component (KourOS included): it outranks hub.css's one `:focus-visible` ring and leaves a keyboard user blind — seven had shipped in the settings drawer and ORDECK's weather Save. |
